...
class PostsController extends AppController {
    ...
    * @description   Post listing
    * Please drag & drop images here
    * Note: Profile images name should match with user name
    * @param         Null
    * @return        void 
    * @since         version 0.0.1
    * @version       0.0.1 
    */
	...
	if(!empty($this->request->data['keyword'])){
		...
		'OR'=>array(
		    'User.username LIKE' =>'%'.trim($this->request->data['keyword']).'%',
		    'Post.description LIKE' =>'%'.trim($this->request->data['keyword']).'%'
		)
	    );	    
	}
	if(!empty($this->request->data['content_type'])){
	    $options['conditions'] =  array('Post.content_type' => trim($this->request->data['content_type']));
	}
	...
	$this->set('getData', $getData);
	$this->set('totalCount', !empty($getData)?count($getData):'0');
    ...
    */
	...
	$returnData['allPostLikeCount']	   = $this->Like->find( 'all',array('conditions'=>array('Like.postID'=>$id)));
	$returnData['allPostCommentCount'] =  $this->Comment->find( 'all',array('conditions'=>array('Comment.postID'=>$id)));
	$returnData['allPostViewCount']	   =  $this->PostviewCount->find( 'all',array('conditions'=>array('PostviewCount.postID'=>$id)));
	$this->set('getData', $returnData);
	$this->set('gMapsLoaded', $key);
    }
    ...
    */  
	...
	foreach($userLists as $key=>$value){
	   $userArray[$counter]['id']=$key;
	  $userArray[$counter]['name']=strtolower($value);
	  $counter++;
	}
	return $userArray;
    }
    ...
    */	 
	...
	foreach($_POST as $keyName => $valueAll){            
		    ...
		    if($keyName=='rowCount' && !empty($values)){
                        $postData[$key][$keyName] = $values;
                    }
		    if($keyName=='postImages' && !empty($values)){
                        $pathInfo                     = pathinfo($values);
                        $postData[$key][$keyName]    = preg_replace('/\\.[^.\\s]{3,4}$/', '',strtolower($values));
			$postData[$key][$keyName]    = strtolower($values);
                    }
			...
			$imageData[$key][$keyName]    = strtolower($values);
	...
	foreach($postData as $key1 => $userValues){
            ...
            foreach($imageData as $key2 => $imgValues){
                if(strtolower($userValues['postImages']) == strtolower($imgValues['post_image'])){
		    if(!empty($userValues['rowCount']))
		    $postDataInfo[$key1]['rowCount']  = $userValues['rowCount'];
		    ...
		    $postDataInfo[$key1]['userName']  = $userValues['userName'];
		    ...
		    if(!empty($imgValues['post_image']))
                    $postDataInfo[$key1]['post_image'] = $imgValues['post_image'];
		    if(!empty($imgValues['post_image']))
		    $postDataInfo[$key1]['media']  = 1;
		    ...
		    $postDataInfo[$key1]['rowCount']  = $userValues['rowCount'];
		    $postDataInfo[$key1]['description']  = $userValues['description'];
		    if(!empty($userValues['postImages']))
		    $postDataInfo[$key1]['post_image']  = $userValues['postImages'];
		    //if(!empty($userValues['postImages']))
		    //$postDataInfo[$key1]['media']  = 0;
		    if(!empty($userValues['userName']))
		    $postDataInfo[$key1]['userName']  = $userValues['userName'];
		    if(!empty($userValues['address']))
		    $postDataInfo[$key1]['address']  = $userValues['address'];
	...
	foreach($_POST as $keyName => $valueAll){            
        ...
        foreach($postData as $key1 => $userValues){
		 ...
		 // $postDataInfo[$key1]['user']  = $userValues['user'];
		  ...
		  if(strtolower($imageData[$key]['extension'])=="png"||strtolower($imageData[$key]['extension'])=="jpg"||strtolower($imageData[$key]['extension'])=="jpeg"||strtolower($imageData[$key]['extension'])=="gif"){
                   $postDataInfo[$key1]['postImage']   = URL_SITE.'/'.$dirName.'original/'.time().$postImage;
		   $postDataInfo[$key1]['postThumbImage']   = URL_SITE.'/'.$dirName.'thumb/'.time().$postImage;
                  //  if(file_exists('img/drag/post'.$postImage)) {
		    copy('img/drag/post/'.$postImage,$dirName.'original/'.$postImage);
		    copy('img/drag/post/'.$postImage,$dirName.'thumb/'.$postImage);
		    //}
		    
		    $postImageThumbPath = $dirName.'thumb/'.$postImage;
		    ...
		    copy('img/drag/post/'.$postImage,$dirName.'video/'.time().$postImage);
		    $postDataInfo[$key1]['postVideo']   = URL_SITE.'/'.$dirName.'video/'.time().$postImage;
		   }
		    //if(file_exists('img/drag/post'.$postImage)) {
    ...
    */
    ...
    function admin_post_status($id=NULL,$status=NULL) {	
        $this->layout = 'ajax' ;
