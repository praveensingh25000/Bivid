...
///////////////////////////////////////////////////////////////////////////////
	function ebayGenerateGalleryImage( $IMAGE01 = '', $IMAGE02 = '', $SAVE = '' ,$chanelaccount='') {
		
		$RAWDIR = WWW_ROOT.'uploads'.DIRECTORY_SEPARATOR.'store'.DIRECTORY_SEPARATOR.'freesize'.DIRECTORY_SEPARATOR;
		$SAVEDIR = WWW_ROOT.'uploads'.DIRECTORY_SEPARATOR.$chanelaccount.DIRECTORY_SEPARATOR;
		...
		if ($SAVE) {
			$save_filename = $SAVEDIR.$SAVE;
		} else {
			$save_filename = '';
		}
		
		$TMP_IMG1 = WWW_ROOT.'tmp/panel.ebay.template.creator.tmpimg.01.'.rand(100,999).'.jpg';
		$TMP_IMG2 = WWW_ROOT.'tmp/panel.ebay.template.creator.tmpimg.02.'.rand(100,999).'.jpg';
		@unlink($TMP_IMG1);
		@unlink($TMP_IMG2);
		...
		$img = imagecreatefromjpeg($RAWDIR.$IMAGE01);
		...
		imageJPEG($newimg,$TMP_IMG1,100);
		
		$this->Thumbnail($TMP_IMG1);
		$this->allow_enlarge = true;
		$this->size(195, 350);
		$this->process();
		$this->save($TMP_IMG1);
		...
		$img = imagecreatefromjpeg($RAWDIR.$IMAGE02);
		...
		imageJPEG($newimg,$TMP_IMG2,100);
		
		$this->Thumbnail($TMP_IMG2);
		$this->allow_enlarge = true;
		$this->size(195, 350);
		$this->process();
		$this->save($TMP_IMG2);
		...
		////////////////////////////////////////////////////////////////////////////////////////////////////////
		// Join the two images into one.                                                                      //
		////////////////////////////////////////////////////////////////////////////////////////////////////////
		
		
		$src1 = imagecreatefromjpeg($TMP_IMG1);
		$info1 = getimagesize($TMP_IMG1);
		
		$src2 = imagecreatefromjpeg($TMP_IMG2);
		$info2 = getimagesize($TMP_IMG2);
		...
		$x1 = (190-$info1[0]-intval((190-$info1[0])/2));
		$y1 = (350-$info1[1]-intval((350-$info1[1])/2));
		$x2 = (190-$info2[0]-intval((190-$info2[0])/2));
		$y2 = (350-$info2[1]-intval((350-$info2[1])/2));
		
		imagecopy($canvas, $src1, 3+$x1, 50+$y1, 0, 0, $info1[0],$info1[1]);
		imagecopy($canvas, $src2, 202+$x2, 50+$y2, 0, 0, $info2[0],$info2[1]);
		
		Header("Content-Type: image/jpeg");
		if ($save_filename) {
			imageJPEG($canvas, $save_filename);
		} else {
			imageJPEG($canvas);
		}
	...
	function ebayGenerateGalleryImage_Combos( $CASE_FILE = '', $COMBO_FILE = '', $ref = '', $save = '' ) {
		
		$SAVEDIR = '/home/not3d/admin/panel/images-raw/_new_gallery_combo/';
		
		$COMBO = imagecreatefromjpeg($COMBO_FILE);
		$CASE = imagecreatefrompng($CASE_FILE);
		...
		list($og_w, $og_h) = getimagesize($CASE_FILE);
		...
		if ($ratio_w > $ratio_h) {
			$tn_w = $max_w;
			$tn_h = intval($og_h/$ratio_w);
		} else {
			$tn_w = intval($og_w/$ratio_h);
			$tn_h = $max_h;
		}
		...
		$pad_h = 0;
		if ($max_w != $tn_w) {
			$pad_w = intval(($max_w - $tn_w) / 2);
		} elseif ($max_h != $tn_h) {
			$pad_h = intval(($max_h - $tn_h) / 2);
		}
		
		$SIZED = imagecreatetruecolor($tn_w, $tn_h);
		$backgroundColor = imagecolorallocate($SIZED, 255, 255, 255);
		imagefill($SIZED, 0, 0, $backgroundColor);
		
		imagecopyresampled($SIZED, $CASE, 0, 0, 0, 0, $tn_w, $tn_h, $og_w, $og_h);
		
		imagecopy($COMBO, $SIZED, ($loc_x + $pad_w), ($loc_y + $pad_h), 0, 0, $tn_w, $tn_h);
		
		if ($save) {
			imageJPEG($COMBO, $SAVEDIR.$save);
		} else {
			imageJPEG($COMBO);
		}
