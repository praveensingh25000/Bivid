        ...
        callbacks:{
            onTotalScroll:function(){                
                ...
                jQuery.ajax({
                    type: "POST",
                    data:"hours="+hours,
                    url : URL_SITE+"/admin/users/index_ajax/"+limitCounter,		
...
function functionGetUserDetail(id,key){
    if(id){
        jQuery('#loader').addClass('loader');
        jQuery('#get_user_detail_data').removeClass('active').html('').hide();
        jQuery.ajax({
            type: "POST",
            url : URL_SITE+"/admin/users/index_detail/"+id+'/'+key,		
            success: function(msg){
                jQuery('#loader').removeClass('loader');
                jQuery('#get_user_detail_data').addClass('active').html(msg).show();
                console.clear();
            }							
        });	
    }
